...
function lorem(...args) {
    ...
    if (sentencesMode) {
        const sentence = convertToNext(array(wordArbitrary, { minLength: 1, size: 'small' })).map(wordsToSentenceMapper, wordsToSentenceUnmapperFor(wordArbitraryNext));
        return convertFromNext(convertToNext(array(convertFromNext(sentence), { minLength: 1, maxLength: maxCount, size })).map(sentencesToParagraphMapper, sentencesToParagraphUnmapper));
    }
    else {
        return convertFromNext(convertToNext(array(wordArbitrary, { minLength: 1, maxLength: maxCount, size })).map(wordsToJoinedStringMapper, wordsToJoinedStringUnmapperFor(wordArbitraryNext)));
    }
}
export { lorem };
