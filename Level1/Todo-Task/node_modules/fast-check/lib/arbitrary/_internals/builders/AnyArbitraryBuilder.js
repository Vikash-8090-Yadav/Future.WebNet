...
function mapOf(ka, va, maxKeys, size, depthIdentifier) {
    return (0, Converters_1.convertFromNext)(entriesOf(ka, va, maxKeys, size, depthIdentifier).map(ArrayToMap_1.arrayToMapMapper, ArrayToMap_1.arrayToMapUnmapper));
}
function dictOf(ka, va, maxKeys, size, depthIdentifier) {
    return (0, Converters_1.convertFromNext)(entriesOf(ka, va, maxKeys, size, depthIdentifier).map(KeyValuePairsToObject_1.keyValuePairsToObjectMapper, KeyValuePairsToObject_1.keyValuePairsToObjectUnmapper));
}
function setOf(va, maxKeys, size, depthIdentifier) {
    return (0, Converters_1.convertFromNext)((0, Converters_1.convertToNext)((0, uniqueArray_1.uniqueArray)(va, { maxLength: maxKeys, size, comparator: 'SameValueZero', depthIdentifier })).map(ArrayToSet_1.arrayToSetMapper, ArrayToSet_1.arrayToSetUnmapper));
}
